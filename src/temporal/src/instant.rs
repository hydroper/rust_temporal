use super::{Duration, ZonedDateTime, RangeError, TimeZone, Calendar, ZonedDateTimeOptions};
use std::ops::{Add, AddAssign, Sub, SubAssign};

use crate::core::validate_epoch_nanoseconds;
...
impl Instant {
    pub fn from_epoch(epoch_duration: Duration) -> Self {
        if validate_epoch_nanoseconds(epoch_duration.nanoseconds()).is_err() {
            panic!("Instant outside of supported range");
        }
        Self { epoch_nanoseconds: epoch_duration.nanoseconds() }
    }
